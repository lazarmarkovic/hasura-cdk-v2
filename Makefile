...
build: install-dep build-actions build-cdk
install-dep:
...
bootstrap: 
	cd cdk; AWS_ACCOUNT_ID=${AWS_ACCOUNT_ID} AWS_REGION=${AWS_REGION} HOSTED_ZONE_NAME=${HOSTED_ZONE_NAME} HOSTED_ZONE_ID=${HOSTED_ZONE_ID} HASURA_HOSTNAME=${HASURA_HOSTNAME} ACTIONS_HOSTNAME=${ACTIONS_HOSTNAME} APP_NAME=${APP_NAME} npx cdk bootstrap aws://${AWS_ACCOUNT_ID}/${AWS_REGION} --profile ${AWS_PROFILE}


deploy: deploy-cdk post-deploy-cdk
deploy-cdk:
